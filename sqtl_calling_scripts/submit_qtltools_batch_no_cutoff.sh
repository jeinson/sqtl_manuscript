...
# out_name="qtltools_results/$out_dir/${tiss}.qtltools.nominal.MAF05.$chr.$date.txt"
...
# sbatch --job-name hap_conf_${TISS}_$i -t 1:00:00 --mem=10g \
# -e slurm_logs/${tiss}.qtltools.full.$chr.$date.stderr.txt \
# -o slurm_logs/${tiss}.qtltools.full.$chr.$date.stdout.txt \
# --wrap="
# qtltools cis \
# --vcf $gtex_vcf \
...
# --nominal 1 \
# --region $chr \
# --out $out_name; gzip $out_name"

# done

sbatch --job-name sQTLM_nom_${TISS}_%A -t 1:00:00 --mem=10G \
-e slurm_logs/${tiss}.qtltools.%a.full.$date.stderr.txt \
-o slurm_logs/${tiss}.qtltools.%a.full.$date.stdout.txt \
--array=1-22 \
--wrap=" 
qtltools cis \
--vcf $gtex_vcf \
...
--out qtltools_results/$out_dir/${tiss}.qtltools.nominal.MAF05.chr\${SLURM_ARRAY_TASK_ID}.$date.txt"
