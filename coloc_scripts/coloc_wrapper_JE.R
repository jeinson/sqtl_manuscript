...
sample_sheet <- readLines("/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/coloc_v8_anno/sample_sheets/sQTL_gwas_coloc_gtex_final_list.tsv")
sample_sheet <- sample_sheet[123]
arg_list <- strsplit(sample_sheet, "\t", fixed = T)[[1]]

# Master Parameters
TEST_QTL_TRAIT=arg_list[1] #"Artery_Tibial"
TEST_CHR=arg_list[2] #21
TEST_GWAS_TRAIT=arg_list[8] #"EAGLE_Eczema"

# QTL Info
TEST_QTL_FILE=arg_list[3] # sprintf("/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/sQTL_updated/qtltools_results/%s_nominal/%s.qtltools.nominal.MAF05.chr%i.07.30.20.txt.gz", TEST_QTL_TRAIT, TEST_QTL_TRAIT, TEST_CHR)
TEST_SDY_FILE=arg_list[6] #sprintf("/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/coloc/sdY/%s_normalized_PSI_sdY.tsv", TEST_QTL_TRAIT)
TEST_COVARIATES_FILE=arg_list[5] #sprintf("/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/sQTL_updated/covariates/%s_covariates.tsv", TEST_QTL_TRAIT)

samples <- read.csv("/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/sQTL_updated/files/samples_with_n.txt", header=F, sep = " ")
TEST_QTL_N=arg_list[4] #samples$V2[grepl(TEST_QTL_TRAIT, samples$V1)]

# GWAS Info
TEST_GWAS_FILE=arg_list[7] #sprintf("/gpfs/commons/groups/lappalainen_lab/data/gtex/v8/gwas/imputed_%s.txt.gz", TEST_GWAS_TRAIT)
TEST_TYPE=arg_list[11] #"cc"
TEST_GWAS_N=arg_list[9] #21399
TEST_CASES=arg_list[10] #13550
...
FIG_OUTDIR="/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/coloc_v8_anno/figures"
...
parser$add_argument("--gwas_type", default=TEST_TYPE, 
...
parser$add_argument("--gwas_cases", default=TEST_CASES, #, type = "integer", # This may be messing things up
                    help="If provided, the number of cases in a case control study")
...
parser$add_argument("--fig_outdir", default=FIG_OUTDIR, 
                    help="The directory where figures are saved")
...
qtl_header <- read_lines("/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/sQTL_v8_anno/files/qtltools_nominal_header_for_coloc.txt")
...
gwas$gwas_s <- as.integer(args$gwas_cases) / args$gwas_n 
...
min_gwas_p <- tapply(dat$gwas_pval, dat$phenotype_id, min)
...
source("/gpfs/commons/groups/lappalainen_lab/jeinson/projects/modified_penetrance/coloc/active_scripts/functions_run_coloc_JE.R")
...
# Do this to make my input file compatible with Silva's script
dat <- rename(dat, "pos" = "position")
dat$variant_id <- dat$id

# Run the coloc wrapper function in parallel
...
run_coloc(phenotype = i, data = dat,
		  ...
		  locuscompare = TRUE,
		  locuscompare_fig = paste0(args$fig_outdir, "/", args$gwas_trait, "_", args$qtl_trait, "_", "locuscompare"),
		  locuszoom = TRUE,
		  locuszoom_fig = paste0(args$fig_outdir, "/", args$gwas_trait, "_", args$qtl_trait, "_", "locuszoom"),
		  plot_main = paste0(args$gwas_trait, " GWAS and ", args$qtl_trait),
		  ld_show = TRUE,
		  ld_variant = "qtl",
		  p1 = p1used, p2 = p2used, p12 = p12used,
		  sensitivity = TRUE,
		  sensitivity_fig = paste0(args$fig_outdir, "/", args$gwas_trait, "_", args$qtl_trait, "_", "sensitivity"),
