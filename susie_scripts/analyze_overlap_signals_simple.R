# analyze_overlap_signals_simple.R
# 
# Keep things simpler in this script. I just want to ask if sQTLs that share
# a causal variant with an eQTL are more likely to affect non-symmetric
# exons. Either symmetric exon groups overall, or credible sets that contain
# at least one non-symmetric exon. 
# 
# I will need to read in the overlapping variant files, then all sQTLs per 
# tissue. 
...
library(cowplot)
svg("figures/cross.tissue.complete.exon.symmetry.fig.svg", width = 6, height = 4)
plot_grid(fisher.test.result.figure, n_per_tiss_barplot, 
          ncol = 2, rel_widths = c(4, 1), align = "h")
dev.off()

 # Calculate the expected percentage of symmetric exons real quick. 
barplot(prop.table(table(unlist(all_css$exon_lengths) %% 3)), 
        col = c("cornflowerblue", "firebrick", "firebrick"), 
        main = "Expected distribution of symmetric exons", 
        xlab = "Exon length mod 3", 
        ylab = "Proportion of all sExons")
symmetric_exon_expt_pct <- 
  prop.table(table(unlist(all_css$exon_lengths) %% 3 == 0))
